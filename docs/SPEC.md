...
The flag is copied onto the follow-up instance; without that the series would fall back to the due-date grid from its second run on, and it would do so silently, because the follow-up looks complete either way. The completion day is read in the household's own zone (`TZ`, see `server/utils/timezone.js`): ticking a task off at 00:30 must count as the new day, or a weekly task would come back six days later. The anchor is local to Juju and does not travel over CalDAV, because RFC 5545 has no way to express it and a mirrored VTODO carries the rule alone. The shared calculation lives in `nextDueAfterCompletion()` in `server/services/recurrence.js`, deliberately separate from the route because resettable countdowns want the same "counts from the moment you touched it" arithmetic (#647).
...
- **CalDAV:** mirrored from `CATEGORIES` on VTODO objects and pushed back. `CATEGORIES` is only in the managed property set because Juju holds the **complete** list — mirroring a single value would make every push delete the tags the server knows and Juju never saw.
...
longer returns, so removing an event in iCloud/Nextcloud also removes it in Juju. Deletion is scoped
...
Tombstones for events deleted in Juju that still exist at the provider (migration v103, #593).
...
- A change is a **patch of the original object**, never a rebuild: only the mirrored properties
  ...
  visibility and subtasks are Juju-internal and never trigger a push.
...
- **Lossy mappings are held from both ends.** Juju has four priority levels and three statuses,
...
Juju now keeps the structured `N` components (family, given, additional, prefix, suffix) and
...
`DB_ENCRYPTION_KEY` (SQLCipher) is strongly recommended. Juju is **not** a medical device and
...
- **CalDAV Multi-Account:** Connect multiple CalDAV servers (iCloud, Nextcloud, Radicale, Baikal) with per-account calendar selection via checkboxes, two-way sync (tsdav), optional outbound target selection per event. **Deleting, editing or moving a synced event in Juju reaches the server too (v1.52.0 · #593),** using the same queue-then-sync mechanism as Google. Two things differ, because CalDAV has no per-event API: a change is a PUT of the whole calendar object, so Juju *patches* the original instead of rebuilding it — only the mirrored properties are swapped, while attendees, alarms, categories and any `RECURRENCE-ID` exception in the same object stay byte-for-byte intact. And since CalDAV cannot move an object between collections, switching the target calendar is create-in-destination followed by delete-in-source, in that order: if the delete fails the event exists twice, which is recoverable, whereas the reverse order could lose it. An event whose original object was not fetched in the current run is deferred rather than rebuilt from Juju's fields alone, which would silently drop everything the server knows and Juju does not. As with Google, the change is attempted immediately on save: the immediate attempt fetches only the affected object rather than whole calendars, so a delete is a single DELETE on the stored URL and an edit one targeted GET plus PUT. Events synced before migration v106 have no stored URL yet — for them the immediate attempt does nothing and the next sync run, which reads the calendar anyway, resolves the URL and applies the change
...
- Prefer `/api.js` for Juju REST calls. It prefixes `/api/v1`, sends session credentials, refreshes CSRF tokens, and uses non-cached fetches for user data.
