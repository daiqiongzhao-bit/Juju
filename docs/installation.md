...
```bash
curl -O https://raw.githubusercontent.com/daiqiongzhao-bit/Juju/main/podman-compose.yml
curl -O https://raw.githubusercontent.com/daiqiongzhao-bit/Juju/main/.env.example
cp .env.example .env  # set SESSION_SECRET and DB_ENCRYPTION_KEY
...
Docker pulls `ghcr.io/daiqiongzhao-bit/Juju:latest` automatically. No build step, no Node.js installation needed.
...
> set `image: ghcr.io/daiqiongzhao-bit/Juju:2.0.1` in your compose file and bump it
