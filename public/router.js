...
function renderChangelog(panel, payload) {
  ...
  const updateRow = panel.querySelector('#changelog-update-row');
  if (updateRow) {
    updateRow.replaceChildren();
    if (updateAvailable) {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.id = 'btn-update-now';
      btn.className = 'btn btn--primary';
      btn.textContent = t('changelog.updateNow');
      btn.addEventListener('click', () => onUpdateNowClicked(latestVersion));
...
function onUpdateNowClicked(targetVersion) {
  const target = String(targetVersion || '').trim();
  if (!target) return;
  const fromEl = document.getElementById('changelog-current-version');
  const from = fromEl ? fromEl.textContent.trim() : '';
  const ok = window.confirm(
    t('changelog.updateConfirm', { from: from || t('changelog.unknownVersion'), to: versionText(target) })
  );
  if (!ok) return;
  startUpdate(target);
}

let __updatePolling = false;
function showUpdateOverlay(target) {
  if (!document.getElementById('update-overlay-style')) {
    const st = document.createElement('style');
    st.id = 'update-overlay-style';
    st.textContent = [
      '.update-overlay{position:fixed;inset:0;background:rgba(0,0,0,.55);display:flex;',
      'align-items:center;justify-content:center;z-index:9999}',
      '.update-overlay[hidden]{display:none}',
      '.update-overlay__box{background:#fff;padding:24px 28px;border-radius:14px;',
      'text-align:center;max-width:320px;box-shadow:0 10px 40px rgba(0,0,0,.3)}',
      '.update-overlay__spinner{width:32px;height:32px;border:3px solid #ddd;',
      'border-top-color:#007aff;border-radius:50%;margin:0 auto 14px;',
      'animation:updspin .8s linear infinite}',
      '@keyframes updspin{to{transform:rotate(360deg)}}',
      '.update-overlay__text{margin:0;color:#1c1c1e;font-size:15px;line-height:1.4}',
    ].join('');
  ...
  if (!ov) {
    ...
    ov.innerHTML = '<div class="update-overlay__box">'
      + '<div class="update-overlay__spinner" aria-hidden="true"></div>'
      + '<p class="update-overlay__text"></p></div>';
    document.body.appendChild(ov);
  }
  ov.querySelector('.update-overlay__text').textContent = t('changelog.updating', { version: versionText(target) });
  ov.hidden = false;
}
function updateOverlayPhase(phase) {
  ...
  if (!ov) return;
  const map = {
    fetching: t('changelog.phaseFetching'),
    'checking-out': t('changelog.phaseCheckingOut'),
    building: t('changelog.phaseBuilding'),
    swapping: t('changelog.phaseSwapping'),
  };
  const txt = map[phase];
  if (txt) ov.querySelector('.update-overlay__text').textContent = txt;
}
...
async function startUpdate(target) {
  ...
  try {
    await api.post('/update', { version: target });
  } catch (e) {
    hideUpdateOverlay();
    window.alert(t('changelog.updateFailed'));
    return;
  }
  pollUpdate(target);
}
async function pollUpdate(target) {
  if (__updatePolling) return;
  __updatePolling = true;
  const deadline = Date.now() + 6 * 60 * 1000;
  const tick = async () => {
    if (Date.now() > deadline) {
      __updatePolling = false; hideUpdateOverlay();
      window.alert(t('changelog.updateTimeout'));
      return;
    }
    try {
      const v = await api.get('/version');
      const cur = v && v.data && v.data.version;
      if (cur && versionKey(cur) === versionKey(target)) {
        __updatePolling = false; hideUpdateOverlay();
        window.alert(t('changelog.updateDone'));
        window.location.reload();
        return;
      }
    } catch (e) { /* container restarting */ }
    try {
      const st = await api.get('/update/status');
      updateOverlayPhase(st && st.phase);
    } catch (e) { /* endpoint down during swap */ }
  ...
  openModal({
    ...
    content: `
      ...
      <div class="changelog-modal">
        ...
        <div class="changelog-update-row" id="changelog-update-row"></div>
...
function navItems() {
  ...
  const baseItems = [
    ...
    { path: '/media', label: t('nav.media'), icon: 'film', module: 'media', section: NAV_SECTION.people },
    { path: '/family-memory', label: t('nav.familyMemory'), icon: 'heart', module: 'memory', section: NAV_SECTION.people },
