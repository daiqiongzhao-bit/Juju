...
- 人际关系「人际树」：原「族谱」标签改名为「人际树」并重写。不再只画血缘（亲子/配偶），而是把整张关系网按连通分量自动排成一棵或多棵树，边上标注关系类型；支持滚轮缩放、拖拽平移、逐级折叠、点击节点查看联系人。
- 影视书库 × Emby 实时同步增强：电视剧按集聚合——标记某一集已看/取消时，自动读取该剧在 Emby 的全部集数并聚合出剧集级状态（全部看完 → 已看；部分看完 → 在看；全部未看 → 想看）。
- 手动「同步观看状态」支持降级：Emby 侧明确标记为「未看」的条目，会把本地已看/在看降级为想看（此前只升级不降级）。
- Emby Webhook 设置卡片改进：新增「生成密钥」按钮（一键生成随机密钥）；保存密钥后自动把「Webhook 接收地址」更新为带 `?secret=…` 的完整地址，便于直接复制到 Emby；新增「最近接收时间」显示。

### 变更

- Webhook 未匹配事件日志由 info 降为 debug，减少日志噪音。

### 数据库

- 迁移 v142：`system_media_config` 增加 `emby_webhook_last_received`（记录最近一次收到 Webhook 的时间）。

### 维护

- 回填 CHANGELOG：补齐 v0.0.10 与 v0.0.14–v0.0.22 的发布说明（此前缺失，导致发布守卫检查失败）。
- `index.html` 静态资源版本参数同步至 0.0.23。

## [0.0.22] - 2026-09-23

### 修复

- Emby Webhook 自动镜像增强：`item.markplayed` / `item.markunplayed` 在影片不存在于 Juju 时也自动新建条目（已看 / 想看），`watch_date` 与 `progress` 对未看置空；仅 playback 的 doing 仍忽略不建。配合 `item.markunplayed` 即「取消已看」实时同步。

## [0.0.21] - 2026-09-23

### 变更

- Emby Webhook 可观测性增强：记录每个入站事件的 event 名、是否含 UserData、Played 值、进度与判定状态（未知事件附带 payload/item 字段名），便于核对 Emby 插件实际负载字段。

## [0.0.20] - 2026-09-22

### 修复

- Emby Webhook 实时同步修复：新建条目补齐 `creator_uid`（默认最低用户 ID），修复 FOREIGN KEY 报错；取消已观看自动同步为未看（wish），进度与观看日期清空。

## [0.0.19] - 2026-09-22

### 新增

- Emby 实时同步（看完自动更新）：新增公开 Webhook 端点 `/webhook/emby`，对接 Emby「Webhook」插件，无需手动点同步。
- 看完影片/剧集自动标记为「已看」并新建条目、自动匹配 TMDB 海报；开始/暂停时实时写入「在看」进度。
- 支持可选 HMAC-SHA256 密钥校验（设置页可配置 Webhook 密钥）。

### 数据库

- 迁移 v141：`media_item` 增加 `progress` 字段，`system_media_config` 增加 `emby_webhook_secret`。

## [0.0.18] - 2026-09-21

### 新增

- Emby 观看历史导入时按标题+年份自动匹配 TMDB 海报写入封面（并发 5、失败不影响导入）；返回值新增 covers 计数。

## [0.0.17] - 2026-09-21

### 修复

- 修复 Emby 同步查询参数未生效的 Bug（此前每次只拉取默认 10 条）；修复后可正确同步全库观看状态。

### 新增

- 「导入观看历史」：把 Emby 已看/在看的电影和剧集一键导入媒体库（按标题+类型自动去重，年份写入元数据）。入口：设置 → 影视书库数据源 → Emby 卡片。
- 族谱视图交互：新增滚轮缩放（以鼠标为中心，0.3～3 倍）与按住拖拽平移；新增「连线」模式（依次点击两个人物建立关系）；新增「+ 新增人物」。
- 家庭相册绑定文件夹：相册支持「从文件夹导入」，输入服务器本地目录或 SMB 挂载路径，列出图片、勾选导入（直接引用文件，不复制不上传）；新增受保护的图片代理接口（路径映射 /hostfs、防目录穿越、仅限图片扩展名）。容器新增只读挂载 `-v /:/hostfs:ro`。

## [0.0.16] - 2026-09-21

### 修复

- 礼金簿空状态下的「+ 新增记录」按钮与顶部工具栏重复，已移除空状态按钮，保留顶部入口。

## [0.0.15] - 2026-09-21

### 变更

- 移除 Emby 照片浏览功能（入口按钮、弹窗、后端代理路由与样式）。

### 修复

- 修复 i18n 占位符：Emby 同步/测试提示、礼金簿统计（`{x}` → `{{x}}`）。
- 同步完成提示现在正确显示实际数值。
...
- 设置 → 媒体元数据 的 Emby 集成卡片新增「用户」下拉框：留空 = 自动（第一个管理员账号），多账号家庭可在「测试连接」或打开设置页后选择同步哪个成员的观看状态。

## [0.0.13] - 2026-09-21

### 新功能

- 全局搜索（Ctrl+K）补齐三个模块：影视书库、家庭文档、菜谱（含隐私可见性过滤）。
- 影视书库 × Emby/Jellyfin 集成：设置→模块→媒体元数据 中配置服务器地址与 API Key，一键同步「已看/在看」观看状态到本地条目（只升级不降级：看完 > 在看 > 想看，按标题+年份智能匹配）。
- 影视书库新增「Emby 照片」浏览：直接翻看 Emby 照片库（服务器代理 API 与图片，Token 不出服务器）。
- 影视书库成员独立标记：同一部影视，每个家庭成员可分别标记 想看/在看/看完（详情弹窗按人选择）。
- 概览新增两个可选用 widget：「影音书库 · 正在看」（封面横条）与「礼金簿 · 本月合计」（红/白事收支）。
- 礼金簿新增统计视图：按月收支柱状图、年度红白事汇总、本年度人情往来 Top、历年概览（可切换年份）。
...
- 影视书库：新增弹窗重构，按类型自动切换数据源（影视 TMDB / 音乐 iTunes / 书籍 OpenLibrary 与 Google Books）。
- 音乐与书籍元数据：iTunes Search（音乐专辑）、Google Books（书籍），封面统一走服务器代理。
- 媒体数据源设置移入「设置 → 模块 → 媒体元数据」，影视书库头部的 TMDB 按钮移除。
- 人际关系：新增族谱视图，支持世系图（上下）、世系图（左右）、世代列表三种布局，可选起始成员并缩放。
- 礼金簿：新增可见条目汇总条（条数 / 礼金合计），卡片重排（金额右列、类型色条），空状态优化。

### 修复

- 修复新增弹窗表单溢出与滚动体验问题。
- 文档：单文件上限从 5 MB 提升到 100 MB（服务端/前端校验、文本提示同步更新）。
- 预算/AA/仪表盘默认货币从 EUR 改为 CNY（人民币）。

## [0.0.10] - 2026-09-21

### 新增

- 红白事记（gift-ledger）新模块：礼品往来记账。
- 媒体库：批量删除 + 按状态分组。
- 家庭相册（memory）相册视图。
- 全局搜索：Ctrl+K 快捷键 + 最近搜索记录。
...
### Added
- 在线一键更新：在「更新日志」中点「立即更新」，应用会自动拉取 GitHub 最新版本、重建镜像并原子替换正在运行的容器（需容器挂载 Docker 套接字，适用于自托管环境）。
- 新增更新端点 `/api/v1/update`（管理员权限）与进度状态端点 `/api/v1/update/status`，配合前端的更新提示、确认弹窗与轮询。

## [0.0.2] - 2026-08-15

Integrated the relationship manager and added a tree-style shared-contacts view.

### Added
- Relationship manager module: contact network graph, common contacts across users, interaction log and anniversaries.
- Tree-style shared-contacts view with expand/collapse branches and highlighted shared nodes.

## [0.0.3] - 2026-08-15

Made version-update notifications self-sustaining.

### Added
- Automated GitHub release creation on deploy (tag + release notes sourced from CHANGELOG.md).
- Changelog fallback: the running version is treated as latest until a real release exists, so the UI never reports an unknown version.
...
### Added
- Imprint (`impressum.html`) and privacy policy (`datenschutz.html`) pages for the github.com/daiqiongzhao-bit/Juju landing site, linked from the footer of every public page.
...
### Changed
- `docker-compose.yml` now references the pre-built GHCR image (`ghcr.io/daiqiongzhao-bit/Juju:latest`) by default - no local build needed to get started (#25)
