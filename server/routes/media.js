  ...
  try {
    ...
    res.json({
      data: {
        ...
        webhookUrl: '/webhook/emby',
        webhookSecretSet: !!(cfg.emby_webhook_secret && cfg.emby_webhook_secret.length),
      },
    });
  } catch (err) {
...
router.put('/emby/config', (req, res) => {
  try {
    ...
    const vKey = req.body.apiKey;
    const vUser = req.body.userId;
    const vSecret = req.body.webhookSecret;
    const urlVal = vUrl !== undefined ? (String(vUrl).trim().replace(/\/+$/, '') || null) : cfg.emby_url;
    const keyVal = vKey !== undefined ? String(vKey).trim() : cfg.emby_api_key;
    const userVal = vUser !== undefined ? (String(vUser).trim() || null) : cfg.emby_user_id;
    const secretVal = vSecret !== undefined ? (String(vSecret).trim() || null) : cfg.emby_webhook_secret;
    if (urlVal && !/^https?:\/\//.test(urlVal)) {
      return res.status(400).json({ error: 'URL muss mit http(s):// beginnen', code: 400 });
    }
    const newKey = keyVal && keyVal.length ? keyVal : cfg.emby_api_key;
    db.get()
      .prepare(
        `UPDATE system_media_config
         SET emby_url = ?, emby_api_key = ?, emby_user_id = ?, emby_webhook_secret = ?, updated_at = strftime('%Y-%m-%dT%H:%M:%SZ','now')
         WHERE id = ?`
      )
      .run(urlVal, newKey || null, userVal, secretVal, cfg.id);
    const updated = emby.getConfig(db.get());
    res.json({ data: { configured: emby.isConfigured(updated), url: updated.emby_url || '', userId: updated.emby_user_id || '', webhookSecretSet: !!(updated.emby_webhook_secret && updated.emby_webhook_secret.length) } });
  } catch (err) {
