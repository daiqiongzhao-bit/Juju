...
test('GET / liefert die dokumentierten Defaults', async () => {
  ...
  assert.equal(body.data.app_name, 'Juju');
...
test('PUT app_name: gültig -> persist, leer -> Rückfall auf Default', async () => {
  ...
  // Leerer Wert löscht -> GET fällt auf den Default 'Juju' zurück.
  assert.equal((await put({ app_name: '   ' })).body.data.app_name, 'Juju');
  assert.equal((await get()).body.data.app_name, 'Juju');
});
